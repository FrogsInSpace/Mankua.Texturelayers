		...
		if (os->obj->IsSubClassOf(triObjectClassID) ) {
			...
			for(int group=0; group<num_groups; group++) {
				...
				if ( uvw_group->GetActiveStatus() ) {
					...
					for( int i_f=0; i_f<group_uvw_data->num_f; i_f++) {
					...
					for ( int i_v=0; i_v<group_uvw_data->num_v; i_v++ ) {
				...
				if ( compact_tverts ) {
					for ( int i_m=0; i_m<MAX_MESHMAPS; i_m++ ) {
						if ( used_map_channels[i_m] ) {
							...
							UVVert *tVerts = mc->v;
							for ( int i_f=0; i_f<num_t_faces; i_f++ ) {
								for ( int i_v=0; i_v<tvFace[i_f].deg; i_v++ ) 
...
void MultiMapMod::ComputeObjectCRC( MNMesh &mnmesh, float &crc_geom, int &crc_topo ) {
	...
	for ( int i_f=0; i_f<mnmesh.numf; i_f++ ) {
		for ( int i_v=0; i_v<mnmesh.f[i_f].deg; i_v++ ) {
